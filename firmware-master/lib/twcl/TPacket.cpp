...
bool TPacket::build(String packetAsString)	{
	if(packetAsString.charAt(0) == START_BYTE &&
		...
		if(packetAsString.charAt(TPACKET_1ST_SPLIT_POSITION) == SPLIT_BYTE 
			...
			if(msg.get().equals("-1"))	{
				error.set("Can not get MESSAGE part.", TERROR_EXCEPTION);
				return false;
			}
			return true;
		}else	{
			error.set("SPLIT byte error.", TERROR_EXCEPTION);
		}
	}else	{
		error.set("START-STOP byte error.", TERROR_EXCEPTION);
	}
	return false;
}
...
String TPacket::giveMeTextBetween(String s, String before, String after)  {
